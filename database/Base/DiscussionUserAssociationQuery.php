 ...
 *
 * @method     ChildDiscussionUserAssociationQuery leftJoinDiscussionMessageCache($relationAlias = null) Adds a LEFT JOIN clause to the query using the DiscussionMessageCache relation
 * @method     ChildDiscussionUserAssociationQuery rightJoinDiscussionMessageCache($relationAlias = null) Adds a RIGHT JOIN clause to the query using the DiscussionMessageCache relation
 * @method     ChildDiscussionUserAssociationQuery innerJoinDiscussionMessageCache($relationAlias = null) Adds a INNER JOIN clause to the query using the DiscussionMessageCache relation
 *
 * @method     \DiscussionQuery|\ActivityUserAssociationQuery|\DiscussionMessageCacheQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class DiscussionUserAssociationQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related \DiscussionMessageCache object
     *
     * @param \DiscussionMessageCache|ObjectCollection $discussionMessageCache  the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildDiscussionUserAssociationQuery The current query, for fluid interface
     */
    public function filterByDiscussionMessageCache($discussionMessageCache, $comparison = null)
    {
        if ($discussionMessageCache instanceof \DiscussionMessageCache) {
            ...
            return $this
                ...
                ->filterByPrimaryKeys($discussionMessageCache->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByDiscussionMessageCache() only accepts arguments of type \DiscussionMessageCache or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the DiscussionMessageCache relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return $this|ChildDiscussionUserAssociationQuery The current query, for fluid interface
     */
    public function joinDiscussionMessageCache($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('DiscussionMessageCache');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'DiscussionMessageCache');
        }
    ...
    /**
     * Use the DiscussionMessageCache relation DiscussionMessageCache object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \DiscussionMessageCacheQuery A secondary query class using the current class as primary query
     */
    public function useDiscussionMessageCacheQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinDiscussionMessageCache($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'DiscussionMessageCache', '\DiscussionMessageCacheQuery');
    }
